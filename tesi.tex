...
As with any category, we need to specify the morphisms between objects:
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.6]
			\begin{scope}[xshift=-55ex]
                ...
                \draw[thick] (m-1-1.south east) -- (m-5-1.north east) ;
			...
			\begin{scope}
				...
				\draw[thick] (m-1-1.south east) -- (m-5-1.north east) ;
				\draw[thick] (m-5-1.north east) -- (m-5-6.north west) ;
			\end{scope}
...
\begin{example}\
	\begin{itemize}
		...
		\item The fields $\k((t^{1/n}))$ for some $n>0$ give examples of totally ramified extensions of $\k((t))$
		      % The extension $\Q_p(\sqrt{p})/\Q_p$ is totally ramified of degree 2.
		      % More generally, given a complete DVR $(R,\mathfrak{m})$ with field of fractions $F$ and an \textit{Eisenstein polynomial}\[f=x^n+a_{n-1}x^{n-1}+\ldots+a_0\]with a_i\in\mathfrak{m}$ for $i<n$ and $a_0\not\in\mathfrak{m}^2$, the extension $F[x]/(f)$ is totally ramified of degree $n$.
	\end{itemize}
\end{example}
...
\begin{proof}{(Sketch)}
	...
	The first statement can be shown by considering an unramified extension of $K$ contained in $L$, as the procyclic structure of the absolute Galois group of its residue field makes the computation of the corestriction
...
\begin{corollary}
	It follows that the pair \((I,i)\) is unique up to unique isomorphism, by representability together with the Yoneda Lemma.
\end{corollary}
...
\begin{align*}
	\Gamma^{\text{ab}} & = \Hom(\Gamma, \Q/\Z)^*                                    & \text{(by Pontryagin duality)}        \\
	                   ...
	                   & = \varprojlim_n\nolimits \sfrac{K^{\times}}{\K^{\times n}} & \text{(by \ref{prop:Kummer})}        \\
	                   & = \widehat{\K^\times}                                      & \text{(by \ref{cor:KxOpen})}.
\end{align*}
Note that the isomorphism \eqref{eq:CupIso} can be viewed as an isomorphism
	\begin{equation}\label{eq:RecIso}
		\omega_n:\K^\times / \K^{\times n} \to \Gamma^{\text{ab}}/n
	\end{equation}
	using \eqref{prop:Kummer} and the fact that \[\H{1}{\K}{\Zn}=
...
\begin{remark}
	One can show that this has the same properties as the usual reciprocity map that are developed in loc. cit., Ch. 9: for instance,
    by definition of profinite completion $\omega$ has dense image. Moreover, for any finite abelian Galois extension $L / K$ there is a diagram
	...
	as can be checked using the density of the image of $\omega_\K$ and some diagram chasing through \eqref{eq:NormDiag}.
...
\begin{theorem}[Existence Theorem]
	Let $\K$ be a $p$-adic field and fix an algebraic closure $\overline{\K}$. Then, for any finite index subgroup $U\subset\K^\times$
...
\begin{proof}
	...
	\[
		...
		\end{tikzcd}
	\]
	for some finite abelian extension $M/K$ by Galois theory, and this implies that $U=N_{M/\K}(M^\times)$.
	...
	As for uniqueness, given finite abelian extensions $L$ and $M$ of $\K$ with $N_{L/\K}(L^\times)=N_{M/\K}(M^\times)$, the preimages of the open subgroups $\operatorname{Gal}{(\K^\text{ab}/L)}$ and $\operatorname{Gal}{(\K^\text{ab}/M)}$ by $\omega_\K$ are the same. By density of the image of $\omega_\K$, we get $\operatorname{Gal}{(\K^\text{ab}/L)}=\operatorname{Gal}{(\K^\text{ab}/M)}$ and thus $L=M$ again by Galois theory.
\end{proof}
