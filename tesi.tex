...
We denote by $\Hom_G{(A,B)}$ the abelian group of morphisms of $G$-modules between $A$ and $B$, which is a subgroup of $\Hom{(A,B)}$.

Finally, we define the category of $G$-modules $\Gmod$, whose objects are $G$-modules and whose morphisms are morphisms of $G$-modules between them. By the above discussion, $\Gmod$ is equivalent to the category of $\Z[G]$-modules, and it is thus an \textbf{abelian} category; in particular it has enough projective and injective objects.
...
Finally, we observe that $\operatorname{Ind}^G_H{(-)}$ and $\operatorname{CoInd}^G_H{(-)}$ are adjoint functors to the forgetful functor
