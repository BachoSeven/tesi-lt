...
\begin{example}[Profinite Groups]\
	\begin{itemize}
		...
		\item Let $K$ be a field, and fix a separable closure $\Ks$ of $\K$. Then, the absolute Galois group $\Gal{K}$ is profinite: indeed, by writing $\Ks$ is the inductive limit of the finite Galois extensions $(K_i)$ of $K$ contained in $\Ks$ and setting $G_i:=\operatorname{Gal}{(K_i/K)}$, we get a surjective projective system of finite groups whose projective limit is $\Gal{K}$.
		      ...
		      More generally, given a (possibly infinite) Galois extension $L/K$, there is a one-to-one correspondence between the closed subgroups of $\operatorname{Gal}{(L/K)}$ and the intermediate field extensions $K\subset M\subset L$. In this correspondence, \textit{normal} subgroups correspond to Galois extensions, and open subgroups to finite extensions (as they  the closed subgroups of finite index). The idea of introducing a topology on the Galois group in order to restore the correspondence in the infinite case is originally due to Wolfgang Krull.
...
\begin{definition}
	A pro-$p$-group is a profinite group $G$ such that $|G|$ is a power of $p$. Given a profinite group $G$, a \textbf{$p$-Sylow subgroup} is a closed subgroup $P<G$ which is a pro-$p$ group with $[G:P]$ not divisible by $p$.
\end{definition}
...
We now state the properties of the cohomology of profinite groups which are obtained by passage to the limit from those developed in the first chapter. The main philosophy is that we have to require subgroups to be closed (open when finiteness of cosets is necessary), and maps defined on $G$ to be continuous.
\begin{remark}[Properties of $\H{i}{G}{-}$]\
	...
	\begin{itemize}
		...
		\item For any closed subgroup $H<G$ the Shapiro lemma holds with the same proof, and the functorial homomorphisms such inflation, restriction and ``conjugation'' are defined as in \ref{def:InfRes}. If $H$ is closed and normal, the Hochschild-Serre spectral sequence and its consequences are still valid (we can use Lemma \ref{lm:GHInjProf} to adapt the previous proof).
		\item If $U<G$ is open, we can define the corestriction homomorphism $\Cor:\H{i}{U}{A}\to\H{i}{G}{A}$; Theorem \label{thm:CorRes} and its corollaries continue to hold.
