...
% TODO Introduce notation: things like Ab, sep. closure, exponent, uniquely divisible, p-primary component, p-torsion, etc.
...
\begin{definition}
    A \textbf{profinite group} is a topological group which is isomorphic to the projective limit of an inverse system of finite groups.
\end{definition}

Here, the topology on the projective limit of a projective system $(G_i)$ is the one induced by the product topology on $\prod{G_i}$, where we endow each finite group with the discrete topology. This makes $\varprojlim{G_i}$ into a compact, Hausdorff, and totally disconnected topological group (it's a closed subspace of the product).
...
In any topological group, open subgroups are also closed, being the complement of the union of the other cosets. In a profinite group, open subgroups are exactly the closed subgroups of finite index: indeed, if $H$ is a open, then the cosets of $H$ form a finite open cover of $G$, and by compactness there is a finite subcover, so there are only finitely many cosets; conversely a closed subgroup of finite index is the complement of a finite union of closed sets, hence open. Moreover, a subgroup is open if and only if it is a neighborhood of $1$.

Finally, the profinite topology is uniquely determined by the fact that the identity $1\in G$ admits a basis $(\Gamma_i)$ of normal open subgroups of finite index, which are neighborhoods of $1$. We can then identify the projective limit of the finite groups $G / \Gamma_i$ with $G$ itself.

% \begin{example}
%     \item Galois Theory
%     \item closed subgroups
% \end{example}

% \begin{definition}[Supernatural Number]
%     A \textbf{supernatural number} is a formal product of prime powers:
% \end{definition}
...
% Corollaries: open subgroups, finite sub-modules, Torsion
...
% - Properties of cohomology which are obtained by passage to the limit:

% for HS, need Lemma(injective in G => in H, state without proof)
...
\begin{definition}
    ...
    The cohomological dimension of $G$ is thus defined as the supremum of the cohomological $p$-dimensions over all prime numbers:
    \[
        \cd{G}:=\sup_{p}\nolimits\cdp{G}.
    \]
\end{definition}
...
\begin{example}\label{ex:Zhat}
    By the above corollary and Example \ref{ex:Zp}, we get $\cdp{\hat{\Z}}=\cdp{\Zp}=1$ for any prime $p$, which implies $\cd{\hat{\Z}}=1$.
\end{example}
...
\begin{proposition}
    ...
    If $\mu_n$ is the group of all $n$-th roots of unity in a fixed separable closure $\Ks$. Then,
...
% TODO remark on how to get Kummer theory from this explicitly(kernel of map into \mu_n=\Zn WHEN K contains the roots of unity)
