...
We now prove an important result about the cohomology of induced modules:
...
\begin{proof}
    The second statement follows immediately from the definition of the action on $\Ind{G}{A}$, as $\H{0}{G}{\Ind{G}{A}}=\operatorname{CoInd}{G}{A}^G$, and if $f: G\to A$ and $f(x)=f(xg)$ for all $x, g\in G$ then $f$ must be constant.
    Let $C^i:=C^i(G,\Ind{G}{A})$ be the set of inhomogeneous $i$-cochains with coefficients in $\Ind{G}{A}$. If $\alpha$
\end{proof}
